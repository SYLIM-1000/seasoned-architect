...
5. `build-loop-codex` 같은 구현 루프 스킬로 slice를 구현합니다.
...
Codex / Claude Code session 시작 또는 sub agent 시작 시 프로젝트 문맥을 가볍게 안내합니다.

설치된 플러그인이 활성화되어 있고 새 session을 열면 동작 대상이 됩니다.
...
hooks/
  hooks.json
planning/
  2026-07-02-skill-expansion-implementation-plan.md
scripts/
  install-git-hook.sh
